...
<<<<<<< HEAD
    	...
    	try {
            ...
            ResultSet rs = preparedStatement.executeQuery();
            while(rs.next()){
            	if(events.get(0).getTitle().equals("null"))
            		events.remove(0);
            	Calendar c = Calendar.getInstance();
            	c.setTime(rs.getDate("date"));
            	events.add(new Event(rs.getInt("id"),rs.getInt("hostid"),rs.getString("location"),rs.getString("title"),c,rs.getString("description"),rs.getString("myImage")));
            }
            connection.close();
            return events;
        } catch (SQLException ex) {
